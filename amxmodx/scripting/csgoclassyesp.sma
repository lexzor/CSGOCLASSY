...
new bool:ducking[33]
new damage_done_to[33]
new view_target[33]
new bool:admin_options[33][10]
new bool:is_in_menu[33]

new weapons[30][10]=
{	"None","P250","SSG","HE","XM1014","C4",
	"MAC-10","AUG","Smoke","Elite","FiveSeven",
	"UMP-45","SG550","Galil-AR","Famas","USP-s",
	"Glock","AWP","MP7","M249","M3","M4",
	"MP9","SG553","Flashbang","Deagle","SG552",
	"AK47","Knife","P90"}
...
public plugin_init()
{
	...
	register_menucmd(register_menuid("ESP"),keys,"menu_esp")
...
public show_esp_menu(id)
{
	...
	new text[2][]={{"W and S to turn ON and OFF"},{"W and S to turn ON and OFF"}}
	new text_index=get_cvar_num("esp_disable_default_keys")
	if (text_index!=1) text_index=0
	format(menu, 300, "ESP [%s]^n%s^n^n1. Line [%s]^n2. Box [%s]^n3. Name [%s]^n4. Health and armor [%s]^n5. Weapon [%s]^n6. Clip and ammo [%s]^n7. Distance [%s]^n8. Show teammates [%s]^n9. Show aimvector [%s]^n^n0. Exit",
...
public client_PreThink(id){
	...
	if (button==0) return PLUGIN_CONTINUE
	...
	for (new i=1;i<=max_players;i++){ // loop through players
		...
		if (admin_options[i][ESP_ON] && first_person[i] && is_user_connected(i) && admin[i] && (!is_user_alive(i)) && (spec[i]>0) && is_user_alive(spec[i])){ // :)
			...
			if (xp>0.0 && xp<=1.0 && yp>0.0 && yp<=1.0){ // if in visible range
				...
				set_hudmessage(47, 79, 79, floatabs(xp), floatabs(yp), 0, 0.0, get_cvar_float("esp_timer"), 0.0, 0.0, 2)
				...
				if (admin_options[i][ESP_HEALTH_ARMOR]){
					...
					format(health,24,"%d HP and %d AP^n",hp,armor)
				}
