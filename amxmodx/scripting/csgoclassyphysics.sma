...
{	
	new szFile[128], szCSGODir[64], szConfigDir[64];
	get_configsdir(szConfigDir, charsmax(szConfigDir));
	csgo_directory(szCSGODir, charsmax(szCSGODir));
	formatex(szFile, charsmax(szFile), "%s/%s/physics.ini", szConfigDir, szCSGODir);
	if(!file_exists(szFile))
	{
		
#if !defined Half_Life
		
		if(!write_file(szFile, ";Print bellow blocked classes^n ^ngrenade^n"))
		
#else	
	
		if(!write_file(szFile, ";Print bellow blocked classes^n"))
	...
	formatex(g_sMapName, charsmax(g_sMapName), "%s%s%s", "maps/", sMapName, "_csgoclassyphysics.ini");
	...
	while(read_file(szFile, line++, string, charsmax(string), line_len))
	{
